...
const MARKET_COL_CANDIDATES = [
  '市場・商品区分',
  '市場商品区分',
  '市場区分',
];

const CODE_COL_CANDIDATES = [
  'コード',
  '銘柄コード',
];
...
function writeCsv(file, rows) {
  const header = ['tv_symbol', 'yahoo_symbol', 'avg_volume_1y_daily'];
  const lines = [header.join(',')];
  for (const row of rows) {
    lines.push([
      csvEscape(row.tv_symbol),
      csvEscape(row.yahoo_symbol),
      csvEscape(String(row.avg_volume_1y_daily)),
    ].join(','));
  }
...
function writeTxt(file, symbols) {
  fs.writeFileSync(path.resolve(file), symbols.join(','), 'utf8');
}
...
function buildOutputFileName(index, totalFiles) {
  const seq = String(index + 1).padStart(3, '0');
  if (totalFiles === 1) {
    return `${OUTPUT_BASENAME}.txt`;
  }
...
function findJpxExcelUrl(html) {
  ...
  if (scored.length === 0) {
    throw new Error('JPXページからExcelリンクを見つけられませんでした。');
  }
...
async function main() {
    ...
    for (const item of chunk) {
      try {
        ...
        rows.push({
          ...
          avg_volume_1y_daily: avgVolume,
        });
      } catch (err) {
        if (errorCount < MAX_ERROR_LOGS) {
          console.log(`取得失敗: ${item.yahooSymbol} / ${err.message}`);
        } else if (errorCount === MAX_ERROR_LOGS) {
          console.log('取得失敗ログが多いため、以降は省略します。');
        }
        errorCount += 1;
        rows.push({
          tv_symbol: item.tvSymbol,
          yahoo_symbol: item.yahooSymbol,
          avg_volume_1y_daily: null,
        });
      }
    }
  ...
  const validRows = rows
    .filter(row => row.avg_volume_1y_daily !== null)
    .sort((a, b) => b.avg_volume_1y_daily - a.avg_volume_1y_daily);
